...
export class ProdRepo {
	...
	public async useExistingConfig(config_name: string): Promise<this> {
		...
		const stmt = await db.prepare(
			'SELECT * FROM Productions WHERE config_name=@config_name AND timestamp=(SELECT MAX(timestamp) FROM PRODUCTIONS)'
		)
	...
	public static async listExistingConfig() {
		const db_config = {
			filename: env['PROD_DB_PATH'],
			driver: sqlite3.Database
		}
		...
		FROM Productions t1
		JOIN (
			SELECT template_name, config_name, MAX(timestamp) AS latest_timestamp
			FROM Productions
			GROUP BY template_name, config_name
		) t2 ON t1.template_name = t2.template_name
			 AND t1.config_name = t2.config_name
			 AND t1.timestamp = t2.latest_timestamp;
		`)
		const datas = await stmt.all()
		return datas
	}
	...
	public static getTemplateList(template_path: string) {
	...
	public async putOnProduction() {
		...
		try {
			await git.clone(
				local_repo_path + '/' + this.repo_name,
				local_prod_path + '/' + this.repo_name,
				{
					'--branch': this.branch,
					'--depth': 1
				}
